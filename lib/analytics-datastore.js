...
class AnalyticsDatastore extends AnalyticsDatastoreBase {
  ...
  getActiveSession(applicationId, time) {
    ...
    const query = datastore.createQuery(this.namespace, 'Session')
   ...
   *
   * Be sure to remove this function when deploying to app engine!
   *
   * @return {Promise}
   */
  generateData() {
  ...
  _queryDailyUsers(start, end) {
    let query = datastore.createQuery(this.namespace, 'DailyUsers')
      .filter('day', '>=', start)
      .filter('day', '<=', end);
    return datastore.runQuery(query)
    .then((results) => {
  ...
  queryCustomRangeSessions(start, end) {
    const query = datastore.createQuery('analytics', 'DailySessions')
      .filter('day', '>=', start)
      .filter('day', '<=', end);
    return datastore.runQuery(query)
    .then((results) => {
      ...
      let keySymbol = datastore.KEY;
      const {ArcAnalyticsCustomRangeSessions} =
        require('../models/arc-analytics-custom-range-sessions');
      const day = dateFormat(new Date(start), 'yyyy-MM-dd');
      const lastDay = dateFormat(new Date(end), 'yyyy-MM-dd');
      const result = new ArcAnalyticsCustomRangeSessions(day, lastDay);
      result.items = entities.map((entity) => {
        let name = entity[keySymbol].name;
        return new ArcAnalyticsDailyItemResult(name, entity.sessions);
      });
      return result;
  ...
  queryCustomRangeUsers(start, end) {
    const query = datastore.createQuery('analytics', 'DailyUsers')
      .filter('day', '>=', start)
      .filter('day', '<=', end);
    return datastore.runQuery(query)
    .then((results) => {
      ...
      let keySymbol = datastore.KEY;
      const {ArcAnalyticsCustomRangeUsers} =
        require('../models/arc-analytics-custom-range-users');
      const day = dateFormat(new Date(start), 'yyyy-MM-dd');
      const lastDay = dateFormat(new Date(end), 'yyyy-MM-dd');
      const result = new ArcAnalyticsCustomRangeUsers(day, lastDay);
      result.items = entities.map((entity) => {
        let name = entity[keySymbol].name;
        return new ArcAnalyticsDailyItemResult(name, entity.users);
      });
...
class AnalyzerDatastore extends AnalyticsDatastoreBase {
   ...
   * @param {Number} end A long representing a millisecond before midnight for the last
   * computation day.
   * @param {String} keyFormat Database key format as a Date format
   * @param {String} groupName The storage name where to save the result.
   * @return {Promise} Promise will resolve if the data was computed and result saved to the
   * database.
   * @throws ComputationRecordExistsError
   */
