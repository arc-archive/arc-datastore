...
class AnalyticsDatastore extends AnalyticsDatastoreBase {
  ...
  getActiveSession(applicationId, time) {
    ...
    let query = datastore.createQuery(this.namespace, 'Session')
  ...
  queryCustomRangeSessions(start, end) {
    var query = datastore.createQuery('analytics', 'DailySessions')
      .filter('day', '>=', start)
      .filter('day', '<=', end);
    return datastore.runQuery(query)
    .then((results) => {
  ...
  queryCustomRangeUsers(start, end) {
    var query = datastore.createQuery('analytics', 'DailyUsers')
      .filter('day', '>=', start)
      .filter('day', '<=', end);
    return datastore.runQuery(query)
    .then((results) => {
