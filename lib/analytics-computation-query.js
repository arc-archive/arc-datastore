...
class AnalyticsComputationQuery {
   ...
   *
   * @param {Number} toDate Query filter start date
   * @param {Number} fromDate Query filter end date.
   ...
   *
   * @return {Promise<Number>} Resolved promise will return number of users / sessions for
   * given time period.
   */
  query() {
  ...
  /**
   * Function to be implemented by extending classes to compute number of sessions / users
   * based on the database query results.
   *
   * @param {Array<Entity>} entities Data read from the datastore.
   */
  _computeResult() {
    throw TypeError('The `_computeResult` method not implemented in query class.');
  }
}
...
class AnalyticsUserQuery extends AnalyticsComputationQuery {
  ...
  _computeResult(entities) {
    ...
    var users = 0;
    var keySymbol = this.datastore.KEY;
    var uuids = [];
    entities.forEach(function(entity) {
      let name = entity[keySymbol].name;
      let parts = name.split('/');
      if (parts.length !== 2) {
        console.log('The key for daily users is invalid: ', name);
        return;
      }
      let appId = parts[0];
...
class AnalyticsSessionQuery extends AnalyticsComputationQuery {
  ...
  _computeResult(entities) {
    console.log('Has computation results', entities);
    if (!entities || !entities.length) {
      return 0;
    }
